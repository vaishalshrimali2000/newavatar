  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      const url = isEditMode
        ? 'http://192.168.168.133:90/mst/editbrand'
        : 'http://192.168.168.133:90/mst/addbrand';
      ...
      try {
        ...
        if (isEditMode) {
          const tmpEditData = rowData.filter((itm) => {
            return itm.CityID !== itemDetails.BrandID && itm.BrandName === formDetails.BrandName
          });
          isDuplicate = tmpEditData?.length > 0 ? 1 : 0;
        } else {
          const tmpData = rowData.filter((itm) => {
            return itm.BrandName === formDetails.BrandName
          });
        ...
        if (isDuplicate === 1) {
          ...
          if (response.status >= 200 && response.status < 300) {
            // setShowSuccessModal(true);
            toast.success(isEditMode ? 'Brand successfully updated!' : 'Brand successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit Brand' : 'Add Brand'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
            <CFormLabel htmlFor="BrandName">Brand Name:</CFormLabel>
            <CFormInput
              type="text"
              id="BrandName"
              name="BrandName"
