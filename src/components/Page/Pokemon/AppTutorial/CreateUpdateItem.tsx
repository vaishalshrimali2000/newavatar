  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      const url = isEditMode
        ? 'http://192.168.168.133:90/mst/editcategory'
        : 'http://192.168.168.133:90/mst/addcategory';
      try {
        let isDuplicate = 0;
        if (isEditMode) {
        ...
        if (isDuplicate === 1) {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            toast.success(isEditMode ? 'Category successfully updated!' : 'Category successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit Category' : 'Add Category'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
