...
const ItemsTable = () => {
  ...
  const handleDelete = () => {
    axios
      .post('http://192.168.168.133:90/mst/disableantique', {
        PartID: itemToDelete.PartID,
  ...
  const filteredData = data.filter(
    (item) =>
      item.PartID.toString().includes(searchTerm) ||
    ...
    <CCard style={{ margin: '0', padding: '0' }}>
      {showForm ? (
        <ItemsCrudOperations
          ...
          itemDetails={selectedItem || { PartName: '', Description: '', SortOrder: '' }}
            ...
            <CTable>
              <CTableHead style={{ backgroundColor: '#DEDDF7' }}>
                <CTableRow>
                  ...
                  <CTableHeaderCell style={{ textAlign: 'start' }}>Antique Name</CTableHeaderCell>
              ...
              <CTableBody>
                {currentEntries.map((item) => (
                  <CTableRow key={item.PartID}>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.PartID}</CTableDataCell>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.PartName}</CTableDataCell>
