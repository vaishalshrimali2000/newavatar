  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      const url = isEditMode
        ? 'http://192.168.168.133:90/mst/editantique'
        : 'http://192.168.168.133:90/mst/addantique';
      try {
        let isDuplicate = 0;
        if (isEditMode) {
        ...
        if (isDuplicate === 1) {
          toast.error("Antique name already exists!");
        } else {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            toast.success(isEditMode ? 'Antique successfully updated!' : 'Antique successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit Antique' : 'Add Antique'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
            <CFormLabel htmlFor="PartName">Antique Name:</CFormLabel>
            <CFormInput
              type="text"
              id="PartName"
              name="PartName"
