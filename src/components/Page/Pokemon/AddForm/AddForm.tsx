...
const ItemsTable = () => {
  ...
  const apiUrl = 'http://192.168.168.133:90/mst/getdesigners'; // Your API URL
  ...
  const handleDelete = () => {
    axios
      .post('http://192.168.168.133:90/mst/disabledesigner', {
        DsgID: itemToDelete.DsgID,
  ...
  const filteredData = data.filter(
    (item) =>
      item.DsgID.toString().includes(searchTerm) ||
      item.DsgName.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <CCard style={{ margin: '0', padding: '0' }}>
      {showForm ? (
        <ItemsCrudOperations
          ...
          itemDetails={selectedItem || { DsgName: '', Description: '', SortOrder: '' }}
            ...
            <CTable>
              <CTableHead style={{ backgroundColor: '#DEDDF7' }}>
                <CTableRow>
                  ...
                  <CTableHeaderCell style={{ textAlign: 'start' }}>Designer Name</CTableHeaderCell>
              ...
              <CTableBody>
                {currentEntries.map((item) => (
                  <CTableRow key={item.DsgID}>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.DsgID}</CTableDataCell>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.DsgName}</CTableDataCell>
