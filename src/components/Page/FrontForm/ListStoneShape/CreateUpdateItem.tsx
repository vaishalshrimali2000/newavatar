...
const ItemsCrudOperations = ({ isEditMode, itemDetails, onClose, onRefresh, rowData}) => {
  ...
  const validateForm = () => {
    return formDetails.StoneShape && formDetails.SortOrder;
  };

  const handleSubmit = async () => {
    if (validateForm()) {
      const url = isEditMode
        ? `${apiUrl}/editstoneshape`
        : `${apiUrl}/addstoneshape`;

      try {
        let isDuplicate = 0;
        if (isEditMode) {
          const tmpEditData = rowData.filter((itm) => {
            return itm.ShapeID !== itemDetails.ShapeID && itm.StoneShape === formDetails.StoneShape
          });
          isDuplicate = tmpEditData?.length > 0 ? 1 : 0;
        } else {
          const tmpData = rowData.filter((itm) => {
            return itm.StoneShape === formDetails.StoneShape
          });
          isDuplicate = tmpData?.length > 0 ? 1 : 0;
        }
        if (isDuplicate === 1) {
          toast.error("Stone shape name already exists!");
        } else {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            toast.success(isEditMode ? 'Stone shape successfully updated!' : 'Stone shape successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  useEffect(() => {
    const fetchZones = async () => {
      try {
        console.log("PPTagID : " + JSON.stringify(itemDetails));
        
        var url = `${apiUrl}/searchstoneshape/` + itemDetails.ShapeID;
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit Stone Shape' : 'Add Stone Shape'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
            <CFormLabel htmlFor="StoneShape">Stone Shape Name:</CFormLabel>
            <CFormInput
              type="text"
              id="StoneShape"
              name="StoneShape"
              value={formDetails.StoneShape || ''}
              onChange={handleChange}
              placeholder="Enter Stone shape name"
            />
          </CCol>
          <CCol md={6}>
