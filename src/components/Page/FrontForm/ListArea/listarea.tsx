...
const ItemsTable = () => {
  ...
  const filteredData = data.filter(
    (item) =>
      item.AreaID.toString().includes(searchTerm) ||
    ...
    <CCard style={{ margin: '0', padding: '0' }}>
      {showForm ? (
        <ItemsCrudOperations
          ...
          itemDetails={selectedItem || { AreaName: '', Description: '', SortOrder: '' }}
            ...
            <CTable>
              ...
              <CTableBody>
                {currentEntries.map((item) => (
                  <CTableRow key={item.AreaID}>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.AreaID}</CTableDataCell>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.AreaName}</CTableDataCell>
