...
const ItemsCrudOperations = ({ isEditMode, itemDetails, onClose, onRefresh, rowData}) => {
      ...
      try {
        ...
        if (isEditMode) {
          const tmpEditData = rowData.filter((itm) => {
            return itm.CourierID !== itemDetails.CourierID && itm.CourierName === formDetails.CourierName
          });
          console.log("isEditMode tmpEditData : "  +JSON.stringify(tmpEditData.length));
          
          isDuplicate = tmpEditData?.length > 0 ? 1 : 0;
        } else {
          const tmpData = rowData.filter((itm) => {
            return itm.CourierName === formDetails.CourierName
          });
        ...
        if (isDuplicate === 1) {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            toast.success(isEditMode ? 'Courier successfully updated!' : 'Courier successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  useEffect(() => {
    const fetchZones = async () => {
      try {
        console.log("Courier : " + JSON.stringify(itemDetails));
        
        var url = `${apiUrl}/searchcourier/` + itemDetails.CourierID;
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit Courier' : 'Add Courier'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
            <CFormLabel htmlFor="CourierName">Courier Name:</CFormLabel>
            <CFormInput
              type="text"
              id="CourierName"
              name="CourierName"
              value={formDetails.CourierName || ''}
              onChange={handleChange}
              placeholder="Enter courier name name"
            />
          </CCol>
          <CCol md={6}>
