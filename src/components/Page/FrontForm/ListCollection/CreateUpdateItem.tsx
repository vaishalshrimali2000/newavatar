...
const ItemsCrudOperations = ({ isEditMode, itemDetails, onClose, onRefresh }) => {
  ...
  const validateForm = () => {
    return formDetails.CollectionName && formDetails.Description && formDetails.SortOrder;
  };

  const handleSubmit = async () => {
    if (validateForm()) {
      const url = isEditMode
        ? 'http://192.168.168.133:90/mst/editcollection'
        : 'http://192.168.168.133:90/mst/addcollection';
  ...
  useEffect(() => {
    const fetchZones = async () => {
      try {
        ...
        var url = 'http://192.168.168.133:90/mst/searchcollection/' + itemDetails.CollectionID;
