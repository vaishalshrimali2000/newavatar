...
const ItemsCrudOperations = ({ isEditMode, itemDetails, onClose, onRefresh, rowData }) => {
  ...
  const validateForm = () => {
    return formDetails.CollectionName && formDetails.Description ;
  };

  const handleSubmit = async () => {
    if (validateForm()) {
      const url = isEditMode
        ? `${apiUrl}/editcollection`
        : `${apiUrl}/addcollection`;

      try {
        let isDuplicate = 0;
        if (isEditMode) {
          const tmpEditData = rowData.filter((itm) => {
            return itm.CollectionID !== itemDetails.CollectionID && itm.CollectionName === formDetails.CollectionName
          });
          isDuplicate = tmpEditData?.length > 0 ? 1 : 0;
        } else {
          const tmpData = rowData.filter((itm) => {
            return itm.CollectionName === formDetails.CollectionName
          });
        ...
        if (isDuplicate === 1) {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            toast.success(isEditMode ? 'Collection successfully updated!' : 'Collection successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  useEffect(() => {
    const fetchZones = async () => {
      try {
        console.log("collection : " + JSON.stringify(itemDetails));
        
        var url = `${apiUrl}/searchcollection/` + itemDetails.CollectionID;
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit Collections' : 'Add Collections'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
            <CFormLabel htmlFor="CollectionName">Collections Name:</CFormLabel>
            <CFormInput
              type="text"
              id="CollectionName"
              name="CollectionName"
