...
const ItemsTable = () => {
  ...
  const filteredData = data.filter(
    (item) =>
      item.RegionID.toString().includes(searchTerm) ||
    ...
    <CCard style={{ margin: '0', padding: '0' }}>
      {showForm ? (
        <ItemsCrudOperations
          ...
          itemDetails={selectedItem || { RegionName: '', Description: '', SortOrder: '' }}
            ...
            <CTable>
              <CTableHead style={{ backgroundColor: '#DEDDF7' }}>
                <CTableRow>
                  ...
                  <CTableHeaderCell style={{ textAlign: 'start' }}>Region Name</CTableHeaderCell>
              ...
              <CTableBody>
                {currentEntries.map((item) => (
                  <CTableRow key={item.RegionID}>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.RegionID}</CTableDataCell>
                    <CTableDataCell style={{ textAlign: 'start' }}>{item.RegionName}</CTableDataCell>
