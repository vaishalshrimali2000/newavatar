...
const ItemsCrudOperations = ({ isEditMode, itemDetails, onClose, onRefresh, rowData}) => {
  ...
  const validateForm = () => {
    return formDetails.PPTag && formDetails.SortOrder;
  };
      ...
      try {
        ...
        if (isEditMode) {
          const tmpEditData = rowData.filter((itm) => {
            return itm.PPTagID !== itemDetails.PPTagID && itm.PPTag === formDetails.PPTag
          });
          isDuplicate = tmpEditData?.length > 0 ? 1 : 0;
        } else {
          const tmpData = rowData.filter((itm) => {
            return itm.PPTag === formDetails.PPTag
          });
          isDuplicate = tmpData?.length > 0 ? 1 : 0;
        }
        if (isDuplicate === 1) {
          toast.error("PPTag name already exists!");
        } else {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            toast.success(isEditMode ? 'PPTag successfully updated!' : 'PPTag successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  useEffect(() => {
    const fetchZones = async () => {
      try {
        ...
        var url = `${apiUrl}/searchpptag/` + itemDetails.PPTagID;
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit PPTagID' : 'Add PPTagID'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
            <CFormLabel htmlFor="PPTag">PPTag Name:</CFormLabel>
            <CFormInput
              type="text"
              id="PPTag"
              name="PPTag"
              value={formDetails.PPTag || ''}
              onChange={handleChange}
              placeholder="Enter PPTag name"
            />
          </CCol>
          <CCol md={6}>
