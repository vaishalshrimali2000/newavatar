  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      const url = isEditMode
        ? 'http://192.168.168.133:90/mst/editcolor'
        : 'http://192.168.168.133:90/mst/addcolor';
      ...
      try {
        ...
        if (isDuplicate === 1) {
          ...
          if (response.status >= 200 && response.status < 300) {
            // setShowSuccessModal(true);
            toast.success(isEditMode ? 'Color successfully updated!' : 'Color successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  return (
    <CCard>
      <CCardHeader style={{ backgroundColor: '#040430', color: 'white' }}>
        <h5>{isEditMode ? 'Edit Color' : 'Add Color'}</h5>
      </CCardHeader>
      <CCardBody>
        <CRow className="mb-3">
          <CCol md={6}>
            <CFormLabel htmlFor="ColorName">Color Name:</CFormLabel>
            <CFormInput
              type="text"
              id="ColorName"
              name="ColorName"
