  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        if (isDuplicate === 1) {
          toast.error("User type name already exists!");
        } else {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            toast.success(isEditMode ? 'User type successfully updated!' : 'User type successfully created!');
            setTimeout(() => {
              onRefresh();
              onClose();
            }, 1000);
  ...
  useEffect(() => {
    const fetchZones = async () => {
      try {
        ...
        var url = `${apiUrl}/searchusertype/` + itemDetails.UserTypeID;
